...
filename='./idl'+idlnumstring+'/divb-xz-'+idlnumstring+'.dat'
...
for i=0, nx-1 do begin
	for j=0, nz-1 do begin
		if divb(i, j) ge maxcolor then divb(i, j)=colorlevels(ncolors-1)
		if divb(i, j) le mincolor then divb(i, j)=colorlevels(2)
...
contour, divb, x, z, nlevels=ncolors, xrange=[-15.0, 15.0], yrange=[0.0, 6.5], zrange=[mincolor, maxcolor], position=[0.18, 0.18, 0.98, 0.98], xstyle=1, xtitle='x, [kpc]', ystyle=1, ytitle='z, [kpc]', /fill, charsize=1.5
