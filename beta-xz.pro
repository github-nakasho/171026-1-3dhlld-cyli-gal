...
filename='./idl'+idlnumstring+'/xzbeta-'+idlnumstring+'.dat'
...
for i=0, nx-1 do begin
	for j=0, nz-1 do begin
		if log10beta(i, j) ge maxcolor then log10beta(i, j)=colorlevels(ncolors-1)
		if log10beta(i, j) le colorlevels(2) then log10beta(i, j)=colorlevels(2)
endfor
endfor
...
contour, log10beta, x, z, nlevels=ncolors, xrange=[-15.0, 15.0], yrange=[0.0, 6.0], zrange=[mincolor, maxcolor], position=[0.18, 0.18, 0.98, 0.98], xstyle=1, xtitle='x, [kpc]', ystyle=1, ytitle='z, [kpc]', /fill, charsize=1.5
...
contour,Vcolorbar, [0.0, 0.2], colorlevels, levels=colorlevels, position=[0.075, 0.18, 0.1, 0.48], xtickformat='(A1)', xstyle=7, xticks=0, xminor=0, ystyle=1, yticks=4, yminor=1, ytickv=[-1, 0, 1, 2, 3], charsize=1.5, /fill, /noerase
