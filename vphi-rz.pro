...
filename='./idl'+idlnumstring+'/vphi-rz-'+idlnumstring+'.dat'
...
;log10rho=transpose(log10rho)
transvphi=transpose(vphi)
...
for i=0, nx-1 do begin
	for j=0, nz-1 do begin
		if transvphi(i, j) ge maxcolor then transvphi(i, j)=colorlevels(ncolors-1)
		if transvphi(i, j) le mincolor then transvphi(i, j)=colorlevels(0)
endfor
endfor
...
contour, transvphi, x, z, nlevels=ncolors, xrange=[0.0, 60.0], yrange=[0.0, 5.0], zrange=[mincolor, maxcolor], position=[0.18, 0.18, 0.98, 0.98], xstyle=1, xtitle='x, [kpc]', ystyle=1, ytitle='z, [kpc]', /fill, charsize=1.5
...
contour,Vcolorbar, [0.0, 0.2], colorlevels, levels=colorlevels, position=[0.075, 0.18, 0.1, 0.48], xtickformat='(A1)', xstyle=7, xticks=0, xminor=0, ystyle=1, yticks=5, yminor=1, ytickv=[-29, -28, -27, -26, -25, -24], charsize=1.5, /fill, /noerase
