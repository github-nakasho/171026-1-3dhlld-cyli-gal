...
filename='./idl'+idlnumstring+'/z100bphi-'+idlnumstring+'.dat'
...
meanbphi=meanbphi*sqrt(4*!pi)*8.474e-6
meanbphi=meanbphi*1e+6
...
for i=0, nx-1 do begin
	for j=0, ny-1 do begin
		if meanbphi(j, i) gt maxcolor then meanbphi(j, i)=colorlevels(ncolors-1)
		if meanbphi(j, i) lt mincolor then meanbphi(j, i)=colorlevels(2)
endfor
endfor
...
;set_plot, 'ps'
;device, xsize=33, ysize=30, file='./bphi0eps/bphi0-'+idlnumstring+'.eps', /color, /encapsulated

polar_contour, meanbphi, phi, r, nlevels=ncolors, xrange=[-15, 15], yrange=[-15, 15], zrange=[mincolor, maxcolor], position=[0.18, 0.18, 0.98, 0.98], xstyle=1, xtitle='x, [kpc]', ystyle=1, ytitle='y, [kpc]', /fill, charsize=1.5
