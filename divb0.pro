...
filename='./idl'+idlnumstring+'/divb0-'+idlnumstring+'.dat'


openr, lun, filename, /get_lun
readf, lun, divb

divb=alog10(abs(divb))
...
ncolors=64
colorlevels=mincolor+findgen(ncolors)*(maxcolor-mincolor)/(ncolors-1)
...
for i=0, nx-1 do begin
	for j=0, ny-1 do begin
		if divb(j, i) ge maxcolor then divb(j, i)=colorlevels(ncolors-1)
		if divb(j, i) le mincolor then divb(j, i)=colorlevels(2)
endfor
endfor
...
polar_contour, divb, phi, r, nlevels=ncolors, xrange=[-15, 15], yrange=[-15, 15], zrange=[mincolor, maxcolor], position=[0.18, 0.18, 0.98, 0.98], xstyle=1, xtitle='x, [kpc]', ystyle=1, ytitle='y, [kpc]', /fill, charsize=1.5
